...
const buildSprite = () => {
  ...
  svgFiles.forEach(function(file, i) {
    ...
    svgContent += `<symbol id="provider-${file.name}" viewBox="0 0 48 48">${svgFileContent}</symbol>`
  })

  let svg = `<svg xmlns="http://www.w3.org/2000/svg"><defs>${svgContent}</defs></svg>`

  fs.writeFileSync('provider-sprite.svg', svg)
}
...
const buildNodes = () => {
  ...
  fs.writeFileSync(`./provider-nodes.json`, iconNodesStringified);
}
